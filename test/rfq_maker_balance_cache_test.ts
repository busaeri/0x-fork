...
describe(SUITE_NAME, () => {
  ...
  before(async () => {
    ...
    balanceCheckerContract = await BalanceCheckerContract.deployFrom0xArtifactAsync(artifacts.BalanceChecker, provider, { from: deployer, gas: 10000000 }, {});
  ...
  describe("runRfqBalanceCheckerAsync", () => {
    it("correctly updates maker addresses", async () => {
      ...
      const maker1 = await dbConnection
        ...
        .where("maker_balance_chain_cache.makerAddress = :address AND maker_balance_chain_cache.tokenAddress = :token", {
