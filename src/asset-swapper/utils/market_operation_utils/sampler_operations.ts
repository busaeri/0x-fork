...
export class SamplerOperations {
    ...
    private _getSellQuoteOperations(
        ...
        const _sources = (SELL_SOURCE_FILTER_BY_CHAIN_ID as any)[this.chainId]
            .exclude([ERC20BridgeSource.MultiHop, ERC20BridgeSource.Native])
            .getAllowed(sources);
        const allOps = _.flatten(
            _sources.map((source: ERC20BridgeSource): SourceQuoteOperation | SourceQuoteOperation[] => {
                ...
                switch (source) {
                    ...
                    case ERC20BridgeSource.Mooniswap:
                        return [
                            ...((MOONISWAP_REGISTRIES_BY_CHAIN_ID as any)[this.chainId] || [])
                                .filter((r: string) => isValidAddress(r))
                                .map((registry: string) =>
                    ...
                    case ERC20BridgeSource.DodoV2:
                        return _.flatten(
                            ((DODOV2_FACTORIES_BY_CHAIN_ID as any)[this.chainId] || [])
                                .filter((factory: string) => isValidAddress(factory))
                                .map((factory: string) =>
        ...
        return allOps as SourceQuoteOperation<FillData>[];
    }
