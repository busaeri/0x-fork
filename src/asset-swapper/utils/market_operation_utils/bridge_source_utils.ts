...
function getShellsForPair(chainId: ChainId | 8453, takerToken: string, makerToken: string): string[] {
    if (chainId !== ChainId.Mainnet) {
        return [];
    }
    return Object.values((SHELL_POOLS_BY_CHAIN_ID as any)[chainId] || {})
        .filter((c: any) => [makerToken, takerToken].every((t: string) => c.tokens.includes(t)))
        .map((i: any) => i.poolAddress);
}

function getComponentForPair(chainId: ChainId | 8453, takerToken: string, makerToken: string): string[] {
    if (chainId !== ChainId.Mainnet) {
        return [];
    }
    return Object.values((COMPONENT_POOLS_BY_CHAIN_ID as any)[chainId] || {})
        .filter((c: any) => [makerToken, takerToken].every((t: string) => c.tokens.includes(t)))
        .map((i: any) => i.poolAddress);
}

function getMStableForPair(chainId: ChainId | 8453, takerToken: string, makerToken: string): string[] {
    if (chainId !== ChainId.Mainnet && chainId !== ChainId.Polygon) {
        return [];
    }
    return Object.values((MSTABLE_POOLS_BY_CHAIN_ID as any)[chainId] || {})
        .filter((c: any) => [makerToken, takerToken].every((t: string) => c.tokens.includes(t)))
        .map((i: any) => i.poolAddress);
}
