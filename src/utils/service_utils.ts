...
export const serviceUtils = {
    ...
    convertToLiquiditySources(sourceBreakdown: SwapQuoteSourceBreakdown): GetSwapQuoteResponseLiquiditySource[] {
        ...
        const allSingleSources = ((SELL_SOURCE_FILTER_BY_CHAIN_ID as any)[CHAIN_ID] || { sources: [] }).sources.filter(
            (source: ERC20BridgeSource) => source !== ERC20BridgeSource.MultiHop,
        );
        const defaultSingleSourceBreakdown = Object.fromEntries(
            allSingleSources.map((source: ERC20BridgeSource) => [source, new BigNumber(0)]),
        );
